...
async def amireallyalive(event):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "**سورس ليندا يعمل بنجاح**"
...
def jmthonalive_text():
    EMOJI = gvarstatus("ALIVE_EMOJI") or "  ✥ "
    jmthon_caption = "**سورس ليندا يعمل بنجاح**\n"
    jmthon_caption += f"**{EMOJI} اصدار التيليثون :** `{version.__version__}\n`"
    jmthon_caption += f"**{EMOJI} اصدار ليندا :** `{jmthonversion}`\n"
...
ROZ_PIC = "https://graph.org/file/5200c319744f40545a624.mp4"
RAZAN = Config.TG_BOT_USERNAME
ROZ_T = (
    f"**⌯︙بوت ليندا يعمل بنجاح 🤍،**\n"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("السورس") and event.query.user_id == jmub.uid:
            buttons = [
                [
                    Button.url("قنـاة ليندا ⚙️", "https://t.me/FX_CTT"),
                    Button.url("المبرمجه ليندا 👨🏼‍💻", "https://t.me/ELA_V1"),
                ]
            ]
