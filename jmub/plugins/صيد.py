...
async def _(event):
    await event.edit(
        """
أوامر الصيد الخاصة بسورس ليندا : 
    ...
    try:
        ch = await jmub(
            functions.channels.CreateChannelRequest(
                title="ELA_V1 HUNTER - صيد ليندا",
                about="This channel to hunt username by - @ELA_V1 ",
            )
        )
        ...
        if isav == True:
            try:
                ...
                await event.client.send_file(
                    event.chat_id,
                    "https://t.me/ELA_V1gif/2",
                    caption="🐊 ELA_V1 the best 🐊\n- - - - - - - - - - - - - - - - - - - - - - - -\n- UserName: ❲ @{} ❳\n- ClickS: ❲ {} ❳\n- Type: {}\n- Save: ❲ Chaneel ❳\n- - - - - - - - - - - - - - - - - - - - - - - -\nThE KiNgS ❲ @ELA_V1 - @FX_CTT ❳ ".format(
...
async def _(event):
    ...
    try:
        ...
        try:
            ch = await jmub(
                functions.channels.CreateChannelRequest(
                    title="ELA_V1 HUNTER - تثبيت ليندا",
                    about="This channel to hunt username by - @ELA_V1 ",
                )
            )
            ch = ch.updates[1].channel_id
